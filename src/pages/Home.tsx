...
function AdMarquee({ adMessage, maintenance }: { adMessage: string; maintenance: any }) {
  ...
  const handleBannerClick = () => {
    window.open(BETA_FEEDBACK_URL, '_blank');
  };
  ...
  return (
    <div className="px-5 mt-2">
      <motion.div 
        ...
        whileTap={{ scale: 0.98 }}
        className="w-full h-12 bg-white rounded-2xl flex items-center overflow-hidden border border-[#F2F4F6] px-4 gap-3 shadow-sm cursor-pointer hover:border-[#12B886]/30 active:bg-gray-50 transition-all group"
      >
        <div className="flex items-center justify-center w-8 h-8 bg-[#12B886]/10 rounded-xl group-hover:scale-110 transition-transform">
          <Megaphone className="w-4 h-4 text-[#12B886]" />
        </div>
        <div className="flex-1 overflow-hidden">
          <p className="text-[13px] font-bold text-[#8B95A1] whitespace-nowrap animate-marquee group-hover:text-[#12B886] transition-colors">
            {adMessage}
          </p>
        </div>
        <motion.div 
          ...
          className="text-[#ADB5BD] group-hover:text-[#12B886] transition-colors"
        >
  ...
  useEffect(() => {
    ...
    const unsubTransactions = onSnapshot(transactionQuery, (snapshot) => {
      ...
      try {
        ...
        currentMonthTxs.sort((a, b) => (b.date || '').localeCompare(a.date || ''));

        setTotalSpent(currentSpent);
        setMedicalSpent(currentMedical);
        setCurrentMonthTransactions(currentMonthTxs);
        setLastMonthTransactions(lastMonthTxs);

        // Derive recent transactions (top 5 sorted by date desc, then createdAt desc)
        ...
        if (isPetLogDebug()) {
          console.log('[PetLog DEBUG] Home.tsx: Snapshot Summary', {
            ...
            processedTransactions: currentMonthTxs.length,
            currentMonth: currentMonth,
