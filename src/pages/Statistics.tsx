...
import { getTransactionAmount } from '../lib/utils';
...
export default function Statistics() {
  ...
  // Recalculate statistics client-side when transactions or date changes
  useEffect(() => {
    ...
    const endOfMonth = `${currentMonthStr}-${lastDayOfMonth}`;
    const startOfPrevMonth = `${prevMonthStr}-01`;
    const endOfPrevMonth = `${prevMonthStr}-${lastDayOfPrevMonth}`;

    let total = 0;
    let prevTotal = 0;
    const categoryTotals: Record<string, number> = {
      FOOD: 0, MEDICAL: 0, SUPPLIES: 0, GROOMING: 0, INSURANCE: 0, OTHER: 0
    };
    ...
    transactions.forEach(item => {
      ...
      if (type === 'expense' || !type) {
        const amt = getTransactionAmount(item);
        ...
        <div className="bg-white rounded-[28px] overflow-hidden p-7 shadow-sm border border-[#F2F4F6]">
          ...
          {totalSpent > 0 && (
            <button 
              ...
              className="w-full app-button-primary mt-10"
            >
