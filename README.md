...
>
> 복잡하고 일관성 없는 동물병원 영수증 정보를 AI를 활용해 간편하게 파싱하고, 보호자가 직접 확인·기록하는 스마트 지출 관리 시스템입니다.
...
* **보관 처리**: 영수증 원본은 Firebase Storage에 업로드되어 안전하게 보관됩니다.
...
* **분류 보정**: 분류 가중치(Keyword, Context, Sequence)를 적용하여 전후 처치 흐름에 따라 분류 신뢰도를 보정합니다.
...
이 프로젝트는 ChatGPT, Antigravity 등 AI 개발 도구를 적극적으로 활용하여 코드 초안 작성과 오류 원인 분석 속도를 높였습니다.

다만 전체적인 기능 아키텍처, 화면 흐름의 설계, Firebase와의 연동 및 권한 관리, 비정상 입력 값의 예외 처리, 빌드 테스트 및 모바일 기기에서의 실제 사용성 검토는 개발자 본인이 직접 확인하고 검증하며 수정해 나갔습니다.
