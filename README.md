...
>
> PetLog는 복잡한 동물병원 영수증 이미지를 AI로 분석해 병원비 기록 초안을 만들고, 보호자가 직접 확인·수정한 뒤 저장할 수 있도록 만든 웹앱입니다.
...
**PetLog**는 영수증 이미지를 업로드하면 AI가 상호명, 날짜, 결제 금액뿐만 아니라 복잡한 병원비 세부 항목까지 파싱하여 기록 초안을 작성해 주는 웹 서비스입니다. AI의 분석 결과는 최종 데이터로 바로 저장되지 않으며, 보호자가 화면에서 직접 검토하고 수정하여 최종 확정하는 **사용자 확인 기반의 데이터 저장 흐름**을 제공합니다.
...
* **보관 처리**: 영수증 이미지는 Firebase Storage에 저장하고, Firestore 기록에는 이미지 URL 또는 storagePath를 함께 저장해 기록과 연결했습니다. 사용자별 경로를 기준으로 Storage 접근 권한을 분리하는 구조를 고려했습니다.

### 🤖 AI OCR 기반 영수증 분석
* **서버리스 AI 분석**: Vercel Serverless Function 내부에서 Google Gemini 2.5 Flash API를 호출하여 영수증 이미지를 분석합니다.
...
### 🧾 영수증 항목 분류 로직
* **키워드와 항목명 패턴**: 키워드와 항목명 패턴을 기준으로 영수증 세부 항목을 진찰, 검사, 처치, 입원, 수술, 약제 등 주요 카테고리로 분류하도록 구성했습니다.
* **사용자 확인 기반 보완**: AI 분석 결과가 항상 정확하지 않을 수 있다고 보고, 사용자가 화면에서 직접 항목과 금액을 수정할 수 있도록 구성했습니다.
...
* **모션 적용**: 직관적인 차트 애니메이션(Framer Motion)을 적용해 지출 현황 변화를 시각적으로 이해하기 쉽게 표현했습니다.
...
### 🤝 사용자 확인 기반 데이터 저장
AI의 데이터 추출 오류(할인액 누락, 오분류 등)로 인해 부정확한 지출 정보가 그대로 기록되는 것을 예방하기 위해, AI 분석 데이터를 사용자 검토용 Form 필드에 바인딩하여 2차 수정을 거친 후 저장되는 구조(Human-in-the-loop)를 설계 및 구현했습니다.
...
### 🔑 Serverless Function 기반 API 호출
Gemini API Key는 클라이언트 번들에 포함하지 않고, Vercel Serverless Function의 서버 환경변수로 관리했습니다. 프론트엔드는 분석 요청 상태와 결과 표시를 담당하고, API Key는 서버 환경변수로 관리하는 구조를 사용했습니다.
...
AI 분석 결과가 진단이나 처방처럼 보이지 않도록 참고용 문구임을 명시하고, Vercel Serverless Function과 클라이언트 표시 단계에서 금지 표현을 필터링하도록 구성했습니다.

---

## 🤖 7. 개발 방식 (AI-Assisted Development)

이 프로젝트는 ChatGPT, Antigravity 등 AI 개발 도구를 코드 초안 작성, 오류 원인 분석, 문서 정리에 활용했습니다.

다만 최종 기능 구조, 화면 흐름, Firebase 연동, 예외 처리, 빌드 테스트, 모바일 사용성 검토는 직접 확인하며 수정했습니다.
...
### 🏥 의료적 오인 위험을 줄이기 위한 Care Insight 문구 정리
* **문제**: AI 지출 분석 브리핑 기능에서 AI가 임의로 특정 질병을 명시하며 "치료가 시급합니다"와 같은 진단을 내려 의료적 판단으로 오해될 수 있는 표현이 노출될 위험이 존재했습니다.
...
* **해결**: HTML5 Canvas API를 이용하여 긴 변을 최대 2000px로 축소하면서 퀄리티 0.85 수준의 JPEG로 클라이언트 사이드에서 선제 리사이징을 수행하는 로직을 구축했습니다. 이를 통해 테스트 이미지 기준 약 5MB 이미지를 1MB 내외로 줄여 업로드 지연을 완화했습니다.
...
* **지출 추이 분석 모델 고도화**: 사용자 본인의 기록을 기준으로 월별 지출 변화와 카테고리별 비율을 더 자세히 시각화
